...
void LineManager::connect_channel(Channel & channel, LineLabels line) {
	...
	if (this->check_line_status(line) == open) {
		if (&channel == this->currently_connected_channel[A] || &channel == this->currently_connected_channel[B]) { // This means that the channel is already connected to a line.
			error_logger.new_error(Error(WARNING, "Trying to connect a channel that is already connected, nothing is done"));
			return;
		}
		...
		error_logger.new_error(Error(WARNING, "Trying to connect channel to bussy line, nothing is done"));
	}
}
